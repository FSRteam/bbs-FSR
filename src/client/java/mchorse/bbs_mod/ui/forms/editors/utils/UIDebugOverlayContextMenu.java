...
public class UIDebugOverlayContextMenu extends UIContextMenu
{
    ...
    {
        if (shape == ValueDebugElement.SHAPE_CUBE) return Icons.BLOCK;
        if (shape == ValueDebugElement.SHAPE_DIAMOND) return Icons.DIAMOND;
        if (shape == ValueDebugElement.SHAPE_RING) return Icons.CIRCLE;
        if (shape == ValueDebugElement.SHAPE_CROSS) return Icons.ADD;
    ...
    {
        this.xy(context.mouseX(), context.mouseY()).wh(this.column.area.w, this.column.area.h).bounds(context.menu.overlay, 5);
    }
    ...
    protected void renderBackground(UIContext context)
    {
        ...
        if (this.config.xray.get()) UIDashboardPanels.renderHighlight(context.batcher, this.xray.area, Direction.BOTTOM);
        if (this.config.dashed.get()) UIDashboardPanels.renderHighlight(context.batcher, this.dashed.area, Direction.BOTTOM);
    }
}
