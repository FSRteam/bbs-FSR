...
public class CircleKeyframeShapeRenderer implements IKeyframeShapeRenderer
{
    ...
    public void renderKeyframe(UIContext uiContext, BufferBuilder builder, Matrix4f matrix, int x, int y, int offset, int c)
    {
        ...
        for (int i = 0; i < NUM_SEGMENTS; i++)
        {
            ...
            builder.addVertex(matrix, innerX1, innerY1, 0F).setColor(c);
            builder.addVertex(matrix, outerX1, outerY1, 0F).setColor(c);
            builder.addVertex(matrix, outerX2, outerY2, 0F).setColor(c);
            builder.addVertex(matrix, innerX2, innerY2, 0F).setColor(c);
        }
    }
