...
public class ReplayBatchProcessor
{
    ...
    {
        Error error = validatePositionChannels(selected, null);
    ...
    {
        if (!hasPositionChannels(target))
        {
            return Error.NEED_POSITION_CHANNELS;
        }

        Error error = validatePositionChannels(selected, target);
    ...
    private static Error validatePositionChannels(List<VisibleReplay> selected, Replay ignored)
    {
        for (VisibleReplay replay : selected)
        {
            if (replay.replay != ignored && !hasPositionChannels(replay.replay))
            {
                return Error.NEED_POSITION_CHANNELS;
            }
        }
    ...
    private static boolean hasPositionChannels(Replay replay)
    {
        return replay != null
            && replay.keyframes.get("x") instanceof KeyframeChannel
            && replay.keyframes.get("y") instanceof KeyframeChannel
            && replay.keyframes.get("z") instanceof KeyframeChannel;
    }
