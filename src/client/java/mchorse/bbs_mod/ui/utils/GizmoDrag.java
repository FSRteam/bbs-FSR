...
public class GizmoDrag
{
    ...
    /** Compatibility flag retained for FSR callers; the resolved Jacobian owns the unit conversion. */
     ...
     * ({@code view · view⁻¹}). The draw passes may shear that VIEW frame toward
     * the eye, but this method and the drag basis remain orthonormal.
    ...
    /** The removed secondary Euler stack now shares the transform's single mode-aware rotation. */
    ...
    {
        return this.setAdditiveRotationBase(offset);
    }
    ...
    {
        return computeRotateAxes(transform, matrixSampler);
    }
}
