...
public final class ModelPhysicsRuntime
{
    ...
    private static void applyCompiled(Level world, int age, float transition, IModel model, ModelInstance instance, List<ModelPhysicsCache.CompiledChain> compiledChains, Map<String, ModelConstraintsConfig.BoneConstraint> constraints, InstanceState state, Matrix4f baseTransform, Map<String, Float> poseFixByBone)
    {
    ...
    private static void applyChain(Level world, int age, float transition, IModel model, ModelInstance instance, ModelPhysicsCache.CompiledChain chain, Map<String, ModelConstraintsConfig.BoneConstraint> constraints, Map<String, PivotFrame> frames, InstanceState instanceState, Map<String, Float> poseFixByBone)
    {
    ...
    private static void step(Level world, int age, float transition, IModel model, List<String> ids, ModelPhysicsCache.CompiledChain chain, Map<String, ModelConstraintsConfig.BoneConstraint> constraints, Vector3f anchorPosition, Quaternionf anchorRotation, Quaternionf parentRotation, Vector3f targetPosition, List<PivotFrame> chainFrames, ChainState state)
    {
            ...
            for (int i = 1; i < state.pos.length; i++)
            {
                ...
                if (collisions)
                {
                    ...
                    if (lenSq > maxStepSq)
                    {
                        ...
                        int minBX = Mth.floor(minX);
                        int minBY = Mth.floor(minY);
                        int minBZ = Mth.floor(minZ);
                        int maxBX = Mth.floor(maxX);
                        int maxBY = Mth.floor(maxY);
                        int maxBZ = Mth.floor(maxZ);
