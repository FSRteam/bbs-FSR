...
public final class ModelPhysicsWorldCollisions
{
    ...
    public static void resolve(Level level, Vector3f[] pos, Vector3f[] prev, int from, int to, float radius, float friction)
    {
        if (level == null || pos == null || prev == null || from < 0 || to > pos.length || to > prev.length || from >= to || radius <= 0F)
        {
            return;
        }

        List<float[]> boxes = gatherSolidBoxes(level, pos, prev, from, to, radius);
    ...
    private static float deepestPenetration(float px, float py, float pz, float radius, List<float[]> boxes, Vector3f outNormal)
    {
        ...
        {
            float[] AABB = boxes.get(i);

            float cx = clamp(px, AABB[0], AABB[3]);
            float cy = clamp(py, AABB[1], AABB[4]);
            float cz = clamp(pz, AABB[2], AABB[5]);
            ...
            {
                /* Centre is inside the AABB: push out through the nearest face. */
                float dMinX = px - AABB[0];
                float dMaxX = AABB[3] - px;
                float dMinY = py - AABB[1];
                float dMaxY = AABB[4] - py;
                float dMinZ = pz - AABB[2];
                float dMaxZ = AABB[5] - pz;
    ...
    private static List<float[]> gatherSolidBoxes(Level level, Vector3f[] pos, Vector3f[] prev, int from, int to, float radius)
    {
        ...
        for (int x = bx1; x <= bx2; x++)
        {
            for (int y = by1; y <= by2; y++)
            {
                for (int z = bz1; z <= bz2; z++)
                {
                    ...
                    VoxelShape shape = state.getCollisionShape(level, mutable, CollisionContext.empty());
    ...
    public static boolean hasFullCubeInAabb(Level level, BlockPos.MutableBlockPos mutable, int minBX, int minBY, int minBZ, int maxBX, int maxBY, int maxBZ)
    {
        if (level == null)
        {
            return false;
        ...
        for (int x = minBX; x <= maxBX; x++)
        {
            for (int y = minBY; y <= maxBY; y++)
            {
                for (int z = minBZ; z <= maxBZ; z++)
                {
                    ...
                    if (block.isCollisionShapeFullBlock(level, mutable))
                    {
                        return true;
                    }

                    if (!block.getCollisionShape(level, mutable, CollisionContext.empty()).isEmpty())
                    {
                        return true;
                    }
                }
