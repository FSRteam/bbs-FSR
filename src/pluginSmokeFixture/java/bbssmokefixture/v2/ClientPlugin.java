...
public final class ClientPlugin implements BBSPlugin
{
    ...
    public void prepare(BBSPluginContext context)
    {
        ...
        BBSRegistrationResult dashboardPanel = client.dashboardPanels().register(
            BBSDashboardPanelSpec.builder("smoke")
                .title(IKey.raw("Smoke fixture 2.0"))
                .icon(Icons.GEAR)
                .build(),
    ...
    {
        UIText status = new UIText("The upgraded 2.0 panel button has not been pressed.").padding(0, 0);
        UIButton button = new UIButton(IKey.raw("Activate 2.0 panel"), (b) ->
            status.text("Button pressed in upgraded smoke fixture 2.0."));

        return BBSDashboardPanelContent.of(UI.column(10, 16,
            new UIText("FSR Dashboard API smoke fixture 2.0").padding(0, 0),
            new UIText("The v1 panel was replaced in place through the same owner-local id.").padding(0, 0),
            status,
            button
        ));
    }
