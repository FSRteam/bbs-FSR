...
final class BBSAddonRegistryAdapters
{
    ...
    static BBSResourceRegistry resources(BBSAddonRegistrationWindow window, BBSAddonDiagnosticRecord diagnostics, BBSAddonDescriptor descriptor, AssetProvider provider)
    {
        return new BBSResourceRegistry()
        {
            ...
            {
                return registerPack(window, diagnostics, descriptor, provider, sourcePack, false);
            }

            @Override
            public BBSRegistrationResult registerSourcePackFirst(ISourcePack sourcePack)
            {
                return registerPack(window, diagnostics, descriptor, provider, sourcePack, true);
            }
        };
    }

    static BBSFormRegistry forms(BBSAddonRegistrationWindow window, BBSAddonDiagnosticRecord diagnostics, BBSAddonDescriptor descriptor, FormArchitect forms)
    {
        return (id, formType) ->
        {
            String key = stringId(id);

            return window.run(diagnostics, key, "forms", () ->
            {
    ...
    static BBSSettingsRegistry settings(BBSAddonRegistrationWindow window, BBSAddonDiagnosticRecord diagnostics, BBSAddonDescriptor descriptor, File settingsFolder)
    {
        return new BBSSettingsRegistry()
        {
            ...
            {
                return window.run(diagnostics, id, "settings", () ->
                {
    ...
    static BBSClipRegistry clips(BBSAddonRegistrationWindow window, BBSAddonDiagnosticRecord diagnostics, BBSAddonDescriptor descriptor, MapFactory<Clip, ClipFactoryData> cameraClips, MapFactory<Clip, ClipFactoryData> actionClips)
    {
        return new BBSClipRegistry()
        {
            ...
            {
                return registerClip(window, diagnostics, descriptor, cameraClips, id, clipType, data, "camera clip");
            }

            @Override
            public BBSRegistrationResult registerActionClip(Link id, Class<? extends Clip> clipType, ClipFactoryData data)
            {
                return registerClip(window, diagnostics, descriptor, actionClips, id, clipType, data, "action clip");
            }
        };
    }

    static BBSParticleRegistry particles(BBSAddonRegistrationWindow window, BBSAddonDiagnosticRecord diagnostics, BBSAddonDescriptor descriptor, Map<String, String> particleComponents)
    {
        return (id, componentClassName) ->
        {
            return window.run(diagnostics, id, "particles", () ->
            {
    ...
    static BBSNetworkRegistry network(BBSAddonRegistrationWindow window, BBSAddonDiagnosticRecord diagnostics, BBSAddonDescriptor descriptor)
    {
        return new BBSNetworkRegistry()
        {
            ...
            public BBSRegistrationResult registerLegacyServerReceiver(ResourceLocation id, NetworkCompat.ServerReceiver receiver)
            {
                ...
                return window.run(diagnostics, key, "network.legacy", () ->
                {
                    ...
                    return diagnostics.record(BBSRegistrationResult.rejected(key,
                        "legacy raw receiver registration has no addon-owned frozen channel; use registerServerReceiver on the addon broker"));
                });
            }
            ...
            {
                String key = id == null ? "<null>" : id.toString();
                return window.run(diagnostics, key, "network.broker", () ->
                {
                    BBSRegistrationResult capability = requireCapability(diagnostics, descriptor, BBSAddonCapability.NETWORK, key, "network");
    ...
    static BBSEventRegistry events(BBSAddonRegistrationWindow window, BBSAddonDiagnosticRecord diagnostics, BBSAddonDescriptor descriptor, EventBus eventBus)
    {
        return (subscriber) ->
        {
            String key = subscriber == null ? "<null>" : subscriber.getClass().getName();
            return window.run(diagnostics, key, "events", () ->
            {
    ...
    private static BBSRegistrationResult registerPack(BBSAddonRegistrationWindow window, BBSAddonDiagnosticRecord diagnostics, BBSAddonDescriptor descriptor, AssetProvider provider, ISourcePack sourcePack, boolean first)
    {
        String key = sourcePack == null ? "<null>" : sourcePack.getPrefix();
        return window.run(diagnostics, key, "resources", () ->
        {
    ...
    private static BBSRegistrationResult registerClip(
        BBSAddonRegistrationWindow window,
    ...
    {
        String key = stringId(id);
        return window.run(diagnostics, key, "clips", () ->
        {
